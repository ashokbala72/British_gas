...
if data_loaded:
    ...
    try:
        ...
        df_weather["Date"] = pd.to_datetime(df_weather["Date"], errors="coerce")
        merged = pd.merge(daily_energy, df_weather, left_on='Timestamp', right_on='Date')
        ...
        if "Temperature_C" in merged.columns and "Consumption_KWh" in merged.columns:
            ...
            temp_consumption_series = "\n".join(
                f"{row['Date'].date()}: {row['Temperature_C']}°C, {row['Consumption_KWh']:.2f} kWh"
        ...
        if st.button("Forecast My Weather-Aware Usage"):
            with st.spinner("Requesting forecast from OpenAI..."):
                ...
                if forecast_lines:
                    ...
                    fig_forecast = px.line(forecast_df, x="Date", y="Predicted_Consumption_KWh", color_discrete_sequence=["green"], title="Forecasted Daily Consumption (Next 15 Days)")
