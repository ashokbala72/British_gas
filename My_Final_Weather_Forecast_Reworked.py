...
if data_loaded:
    ...
    st.header("🔮 Forecast My Next 15 Days Usage (with Weather)")
    try:
        ...
        forecast_prompt = (
            ...
            f"Format: Date: Temp°C, Condition - Predicted Consumption: X.XX kWh"
        )
        ...
        if st.button("Forecast My Weather-Aware Usage"):
            with st.spinner("Requesting forecast from OpenAI..."):
                ...
                try:
                    ...
                    forecast_lines = re.findall(
                        r"(\d{4}-\d{2}-\d{2}):\s*(\d{2})°C,\s*(.*?)\s*-\s*Predicted Consumption:\s*([\d.]+)\s*kWh",
                        forecast_text
                    )
                    if forecast_lines:
                        forecast_df = pd.DataFrame(forecast_lines, columns=["Date", "Temperature_C", "Condition", "Predicted_Consumption_KWh"])
                        forecast_df["Date"] = pd.to_datetime(forecast_df["Date"])
                        forecast_df["Temperature_C"] = forecast_df["Temperature_C"].astype(float)
                        forecast_df["Predicted_Consumption_KWh"] = forecast_df["Predicted_Consumption_KWh"].astype(float)

                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=forecast_df["Date"], y=forecast_df["Predicted_Consumption_KWh"],
                                                 mode='lines+markers', name='Predicted Consumption (kWh)', yaxis='y1'))
                        fig.add_trace(go.Scatter(x=forecast_df["Date"], y=forecast_df["Temperature_C"],
                                                 mode='lines+markers', name='Forecast Temperature (°C)', yaxis='y2'))

                        fig.update_layout(
                            title="15-Day Energy Forecast: Consumption vs Temperature",
                            xaxis_title="Date",
                        ...
                        st.warning("⚠️ Could not parse forecasted data from AI response.")
                except Exception as parse_err:
                    st.error(f"Failed to parse forecast data: {parse_err}")
    except Exception as e:
